...
getwd()
setwd("C:/Users/jcmartinez/OneDrive - El Colegio de México A.C/5. Proyectos/2023/84. Curso-Colmex-R/2. Code/Codigo/Datos")
---------------------------------------
# NOTA:
...
#3.  Importar datos
    ...
    enoe<-read.csv("ENOEN_SDEMT322.csv") %>% data.frame()
...
#6. Subconjuntos 
    ...
    enoe.1 <-enoe %>%select("sex", "eda","pos_ocu","fac_tri", "upm")
    ...
    enoe.3 <- enoe %>%select("eda","pos_ocu","fac_tri", "upm", "clase2")%>%filter(sex=="Mujer" & clase2==1)
    ...
    enoe.3 <- enoe%>%filter(sex=="Mujer" & clase2==1)%>%select("sex", "eda","pos_ocu","fac_tri", "upm")
